...
void	game_draw_sprites_init(t_drsprite *sprt, t_game *game)
{
	...
	sprt->invdet = 1.0 / (game->player.plane.x * game->player.dir.y - game->player.dir.x * game->player.plane.y);
	sprt->transformX = sprt->invdet * (game->player.dir.y * sprt->spritex - game->player.dir.x * sprt->spritey);
	sprt->transformY = sprt->invdet * (-game->player.plane.y * sprt->spritex + game->player.plane.x * sprt->spritey);
	sprt->spriteScreenX = (int)(((GAME_LX) / 2.0) * (1 + sprt->transformX / sprt->transformY));
...
void	game_draw_sprites(t_game *game)
{
	...
	while (++sprt.x < game->map.nb_entity)
		game->map.entity[sprt.x].dist = ((game->player.pos.x - game->map.entity[sprt.x].pos.x) * (game->player.pos.x - game->map.entity[sprt.x].pos.x) + (game->player.pos.y - game->map.entity[sprt.x].pos.y) * (game->player.pos.y - game->map.entity[sprt.x].pos.y));
	...
	while (++sprt.x <  game->map.nb_entity)
	{
		...
		{
			sprt.texX = (int)(256 * (sprt.stripe - (-sprt.spriteWidth / 2 + sprt.spriteScreenX)) * 512 / sprt.spriteWidth / 256);

			if (sprt.transformY > 0 && sprt.stripe > 0 && sprt.stripe < (GAME_LX) && sprt.transformY < game->Zbuffer[sprt.stripe])
			{
				...
				while (++sprt.y < sprt.drawEndY)
				{
					...
					{
						sprt.angle = atan2(sprt.pos.y, sprt.pos.x) - atan2(game->map.entity_ptr[sprt.x]->dir.y, game->map.entity_ptr[sprt.x]->dir.x) + M_PI_4 / 2;
						if (sprt.angle < 0)
							sprt.angle += 2 * M_PI;
						...
						sprt.i = 0;
					sprt.color = (t_color *) &((Uint8*)(game->map.sprite[game->map.entity_ptr[sprt.x]->texture].tex->pixels))[(int)sprt.texX * 3 + (sprt.texY * 3 * game->map.sprite[game->map.entity_ptr[sprt.x]->texture].tex->w) + (sprt.i * 3 * TEX_SIZE)];
					if (!(sprt.color->r == 0xFF && sprt.color->g == 0x00 && sprt.color->b == 0xFF))
						game_draw_pixel(game, game->sdl.text_buf, GAME_X + GAME_LX - sprt.stripe, GAME_Y + sprt.y, sprt.color);
				}
			}
		}
	}
